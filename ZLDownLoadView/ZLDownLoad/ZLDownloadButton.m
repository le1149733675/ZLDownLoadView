...
#import "ZLStartButton.h"

@interface ZLDownloadButton ()<StartbuttonDelegate>//服从协议
{
...
@property (nonatomic, strong) ZLStartButton *startButton;
@property (nonatomic, strong) UIView *vibrationWaveView;

@end

@implementation ZLDownloadButton

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [UIColor whiteColor];
        [self addSubview: self.vibrationWaveView];//下载后的振动波
...
- (void)initView {
    ...
    self.borderPath = [UIView circlePath:rect lineWidth:0];
...
- (ZLStartButton *)startButton {
    if (!_startButton) {
        _startButton = [[ZLStartButton alloc] initWithFrame:self.frame];
